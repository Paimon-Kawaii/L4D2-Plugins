...
public Action Cmd_GiveAmmo(int client, any args)
{
	if (IsValidClient(client) && IsSurvivor(client))
		ExecuteCommand(client, "give", "ammo");
...
public Action Cmd_JoinSurvivor(int client, any args)
{
	if (IsValidClient(client) && !IsSurvivor(client) && !IsFakeClient(client))
	{
		...
		g_hServerMaxSurvivor.SetInt(g_hServerMaxSurvivor.FloatValue >= survivors ? view_as<int>(survivors) : IsSurvivorTeamFull() ? surPlayerCount + 1
																										 : surPlayerCount		  ? surPlayerCount
																																  : 1);
	...
	for (int client = 1; client <= MaxClients; client++)
		if (IsSurvivor(client) && !IsFakeClient(client))
		{
			...
			if (GetPlayerHealth(client) < 100)
				ExecuteCommand(client, "give", "health");
...
public Action Timer_NoWander(Handle timer, int client)
{
	...
	ExecuteCommand(client, "sb_takecontrol", "");
...
void ResetInventory()
{
	for (int client = 1; client <= MaxClients; client++)
		if (IsSurvivor(client))
		{
			DeleteInventoryItem(client);
			ExecuteCommand(client, "give", "pistol");
		}
}
