...
#define SKEET_LEAPING_AI       (0x02)
#define DEBUFF_CHARGING_AI      (0x04)
#define BLOCK_STUMBLE_SCRATCH   (0x08)
...
new                     iLeapInterrupt                                        = 170;                  // Caches leap interrupt cvar's value
new                     g_iTotalSkeetDmg[MAXPLAYERS+1]                        = { 0, ... };           // How much damage done in a single hunter leap so far
...
public OnPluginStart()
{
    ...
    new Handle:hCvarEnabled = CreateConVar("sm_aidmgfix_enable", "15", "Bit flag: Enables plugin features (add together): 1=Skeet pouncing AI, 2=Skeet leaping AI, 4=Debuff charging AI, 8=Block stumble scratches, 15=all, 0=off", FCVAR_NONE|FCVAR_NOTIFY);
    new Handle:hCvarPounceInterrupt = CreateConVar("z_pounce_damage_interrupt", "150", "How many dmg can you skeet an ai ht");
    new Handle:hCvarLeapInterrupt = CreateConVar("z_leap_damage_interrupt", "170", "How many dmg can you skeet an ai jk");

    HookConVarChange(hCvarEnabled, OnAIDamageFixEnableChanged);
    HookConVarChange(hCvarPounceInterrupt, OnPounceInterruptChanged);
    HookConVarChange(hCvarLeapInterrupt, OnLeapInterruptChanged);

    fEnabled = GetConVarInt(hCvarEnabled);
    iPounceInterrupt = GetConVarInt(hCvarPounceInterrupt);
    iLeapInterrupt = GetConVarInt(hCvarLeapInterrupt);
...
public OnLeapInterruptChanged(Handle:cvar, const String:oldVal[], const String:newVal[])
{
    iLeapInterrupt = StringToInt(newVal);
}
...
public Action:OnTakeDamage(victim, &attacker, &inflictor, &Float:damage, &damagetype)
{
    ...
    if (fEnabled && IsClientAndInGame(victim) && IsClientAndInGame(attacker) && damage > 0.0 && GetClientTeam(victim) == TEAM_INFECTED && IsFakeClient(victim))
    {
        ...
        {
            g_iTotalSkeetDmg[victim] += RoundToFloor(damage);

            // Have we skeeted it?
            if (g_iTotalSkeetDmg[victim] >= iPounceInterrupt)
            {
                // Skeet the hunter
                g_iTotalSkeetDmg[victim] = 0;
                damage = float(GetClientHealth(victim));
                return Plugin_Changed;
            }
        }
        else if (zombieClass == ZC_JOCKEY && (fEnabled & SKEET_LEAPING_AI))
        {
            new abilityEnt = GetEntPropEnt(victim, Prop_Send, "m_customAbility");
            if (IsValidEntity(abilityEnt) && GetEntProp(abilityEnt, Prop_Send, "m_isLeaping") > 0)
            {
                g_iTotalSkeetDmg[victim] += RoundToFloor(damage);

                // Have we skeeted it?
                if (g_iTotalSkeetDmg[victim] >= iLeapInterrupt)
                {
                    // Skeet the jockey
                    g_iTotalSkeetDmg[victim] = 0;
                    damage = float(GetClientHealth(victim));
...
public Action:OnPlayerRunCmd(client, &buttons)
{
    ...
    if ((fEnabled & BLOCK_STUMBLE_SCRATCH) && IsClientAndInGame(client) &&
        GetClientTeam(client) == TEAM_INFECTED && IsFakeClient(client) &&
        SDKCall(hIsStaggering, client)) buttons &= ~IN_ATTACK2;

    if(IsClientAndInGame(client) && IsFakeClient(client) && GetClientTeam(client) == TEAM_INFECTED)
    {
        new zombieClass = GetEntProp(client, Prop_Send, "m_zombieClass");
        if(zombieClass == ZC_JOCKEY && ((buttons & IN_JUMP) || (buttons & IN_ATTACK)))
            g_iTotalSkeetDmg[client] = 0;
    }
...
{
    //If client died, clear its skeeted dmg.
    new client = GetClientOfUserId(GetEventInt(event, "userid"));
