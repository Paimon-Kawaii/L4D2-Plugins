 ...
 * @Last Modified time: 2024-11-17 10:21:13
...
#define VERSION       "2024.11.17#181"

#define LIBRARY_NAME  "si_pool_plus"
#define GAMEDATA_FILE "si_pool_plus"

#include <si_pool_plus>
#include <paiutils>

public Plugin myinfo =
{
    name = "Special Infected Bot Client Pool Plus",
    author = "我是派蒙啊",
    description = "A PLUS version for SIPool. SIPool - A Client Pool for SI Bots, used to avoid lots of CreateFakeClient() operation",
    version = VERSION,
...
DynamicDetour g_ddSpawnSpecial, g_ddConnectClient;
...
{
    int zclass_idx = GetNativeCell(2);
    ...
    return RequestSIBot(zclass_idx, origin, angle);
}
...
int RequestSIBot(int zclass_idx, const float origin[3], const float angle[3])
{
    zclass_idx--;
    ...
    InitializeSpecial(bot, origin, angle);
...
void PrepareSDKCalls()
{
    ...
    CreateDetour(gameData, g_ddSpawnSpecial, DTR_ZombieManager_SpawnSpecial, "L4D2::ZombieManager::SpawnSpecial");
...
MRESReturn DTR_ZombieManager_SpawnSpecial(DHookReturn hReturn, DHookParam hParams)
{
    ...
    int bot = RequestSIBot(class, a1, a2);
    if (!IsInfected(bot)) return MRES_Ignored;
